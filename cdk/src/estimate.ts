 ...
 * fit minus a measured subtraction, which is a projection. ONE pass left the
 * default `editor` expansion (see docs/editor-ablation.md):
 *
 *   editor-pacing per-chapter $72.68 / 328 calls = $0.222/chapter
 *
 * so PER_CHAPTER 1.86 → 1.64, and PER_CHAPTER_SECONDS is scaled by the same
 * ratio (0.881). editor-voice was ALSO removed in an earlier revision of this
 * change and then restored: a blind forced-choice tournament measured a 65.6%
 * reader preference for its output (p=0.004), so its $0.248/chapter stays.
 * RE-FIT BOTH against the first completed post-deletion run rather than trusting
 * these; and note the wall-time constant was already known to UNDER-estimate on
 * edit-heavy runs before this change, which this PR does not attempt to fix.
 *
...
// 0.881 for the one removed pass (see the 2026-08 note above); re-fit after
// the first post-deletion run.
const FIXED_SECONDS = 240;
const PER_CHAPTER_SECONDS = 84;
