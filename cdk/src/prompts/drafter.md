...
- **Alongside step 11:** for every fact you log via `append_continuity`, ALSO call `assert_fact` with a stable `entity` id (a slug like `eira-bowman`), a dotted `attribute` key (`age`, `notebook.count`, `hearing.left_ear`), and the `value` (include a `unit` when the value is a number). Reuse the SAME entity id + attribute key for a given fact across chapters so later chapters reference it consistently. Re-asserting the same entity.attribute within a chapter overwrites it (re-draft idempotency); across chapters it records a new dated assertion — a harmless restatement if the value is unchanged, but if you are deliberately CHANGING an established value, pass `supersedes` with the prior fact id (get it from `query_facts`) so the old value is retired. For every new named entity you log via `append_glossary`, ALSO call `upsert_entity` with a stable slug `id`, a `kind`, and a `display_name`.
