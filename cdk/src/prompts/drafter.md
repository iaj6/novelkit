...
- **Alongside step 11:** for every fact you log via `append_continuity`, ALSO call `assert_fact` with a stable `entity` id (a slug like `eira-bowman`), a dotted `attribute` key (`age`, `notebook.count`, `hearing.left_ear`), and the `value` (include a `unit` when the value is a number). Reuse the same entity id + attribute across chapters so the same fact lands in one slot. For every new named entity you log via `append_glossary`, ALSO call `upsert_entity` with a stable slug `id`, a `kind`, and a `display_name`.
