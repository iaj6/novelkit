...
**You MUST record every specific fact your chapter introduces that a later chapter could contradict into the world store** — `assert_fact` for facts that atomize to entity.attribute=value, or `append_continuity` for free-text statements that don't. (The store is the source of truth; `logs/continuity.md` is regenerated from it after each chapter.) Examples of facts that must be recorded:
