...
**You MUST record every specific fact your chapter introduces that a later chapter could contradict into the world store.** DEFAULT to `assert_fact` — most facts atomize to `entity.attribute = value` (ages, dates, roles, locations, counts, physical attributes, statuses, possessions), and **only an atomized fact can be checked by the continuity audit**. First `upsert_entity` the thing (assert_fact REJECTS an unregistered entity), then assert one fact per `(entity, attribute)`. Reserve `append_continuity` for a genuinely unstructured claim that won't reduce to a single `entity.attribute=value` (a multi-part plot constraint, a thematic rule). (The store is the source of truth; `logs/continuity.md` is regenerated from it after each chapter.) Examples of facts that must be recorded:
...
4. **Pre-write awareness.** Two checks before composing:
   - **Quantitative fact discipline.** Before committing any specific number, date, duration, age, distance, count, or time interval to the page for an established entity (a character, an object, a place, an event that already appears in `logs/continuity.md`), scan the fact ledger for prior assertions about the same entity-attribute pair. If a prior assertion exists, your chapter must be consistent with it — OR, if the fact legitimately CHANGES over story-time (a character ages, an object moves, a person's status/role/location changes), re-assert it with `supersedes` set to the prior fact id (from `query_facts`) so the store reflects the CURRENT state. Two live values for the same entity.attribute read as a contradiction in the continuity audit even when the change is intentional — superseding is exactly how you tell legitimate evolution apart from an error. If the entity is new in your chapter, the number you commit becomes canonical; record it via `assert_fact` (numbers atomize cleanly) so subsequent chapters can match. This is how the most painful cross-chapter contradictions get prevented at draft time — they emerge when chapter N says "two months" and chapter N+2 says "three years" about the same letter (or any equivalent), and they are catastrophically expensive to fix after the manuscript is complete.
...
6. Draft the chapter to hit its outline beats in order. **Treat the outline's word target as an upper guide, not a quota** — write what the beats need and stop. Do not pad to reach a number: stretching a scene, re-describing what the reader already knows, or re-quoting a document to fill space is the failure mode the pacing pass exists to cut. A chapter that lands its beats in fewer words than the target is finished, not short.
...
11. **Record every specific named fact your chapter introduces into the world store** (see the next section for how) — `assert_fact` for structured facts, `append_continuity` for free-text statements. For every new named place, person, object, or term, call `upsert_entity` and `append_glossary`. Over-log; do not under-log.
12. Stop.

## Recording facts into the world store (the source of truth)

Durable facts live in a structured, queryable world store, not in hand-written markdown. `logs/continuity.md` — which you read for prior facts — is REGENERATED from the store after each chapter, so what you record here is exactly what later chapters will see. Recording facts must never change the prose you write.

- **Before you write (before step 7):** call `open_chapter` with this chapter's id. This opens the chapter transaction so the captures below are attributed to it.
- **Step 11 — record facts.** First get a stable entity id: call `upsert_entity` (stable slug `id`, `kind`, `display_name`, any `aliases`) for anything new, or `resolve_entity` to get the existing id — then **always pass that exact id** to `assert_fact`, never a free-typed name. (The store canonicalizes known names/aliases for you, but consistent ids are what let a later chapter's contradiction collide on a key — the thing that makes the structured audit work at all.) Then for every fact call `assert_fact` with that `entity` id, a canonical dotted `attribute` key, and the `value` (a `unit` is REQUIRED when the value is numeric). Follow these rules — they are what make the store usable:
  - **One fact per call.** A compound sentence becomes several `assert_fact` calls, one per `(entity, attribute)`. Do not pack "age 52, harbormaster since 1936, widowed 1935" into one.
  - **Reuse canonical attribute keys** so the same thing keys the same way every chapter. Characters: `age`, `birth_year`, `role`, `origin_place`, `native_language`, `alive`. Places: `located_in`, `fictional`, `construction`. Events: `date`, `location`. Prefer these over synonyms (`age`, not `years_old`/`aged`). If none fits, pick a clear dotted key and stay consistent with it.
  - **Hedged or disjunctive facts still atomize.** "approximately 1891" / "1935 or 1936" get the canonical key with `confidence: provisional` (or `inferred`) and a best single value or range token — NOT a free-text dump.
  - **Changing an established value** (a correction, OR a legitimate evolution over story-time — a character ages, an object moves, a person's status/role/location changes): pass `supersedes` with the prior fact id (from `query_facts`) so the old value is retired and only the current value stays live. Skipping this makes the continuity audit flag your intentional change as a contradiction. An unchanged restatement needs nothing.
- **Verbatim documents (logs, letters, forms).** When your chapter writes a fixed document a later chapter might re-quote — a harbor-log entry, a letter, a form — call `register_record` (a stable `recordId` slug, a `label`, its EXACT text). When a LATER chapter re-quotes a registered document, call `read_record` FIRST and reproduce its text VERBATIM inside the quotation — only the surrounding prose frame is yours to vary. Re-improvising a document's wording (a different time, a changed line) is a continuity break. Also capture the document's load-bearing values as facts on a `document`-kind entity (e.g. `harbor-log.discovery_time`), so a drifted value is caught even when the prose isn't pasted.
- **Useful while drafting:** `query_facts` (what's already established about an entity — cheaper than re-reading the whole ledger), `resolve_entity` (find an entity's stable id by name), and `read_record` (re-read a registered document before re-quoting it).
- **As the final action (after step 11, before you stop):** call `close_chapter`. If it warns that no facts were captured, you forgot the `assert_fact` calls — but it will not block.
...
- Do not modify files in `canon/` (other than via `append_glossary`).
