...
## World store (shadow capture — ADDITIVE, do this too)

The pipeline is migrating the markdown logs above into a structured, queryable world store. During this transition you populate BOTH: the markdown calls above remain authoritative, and you ALSO mirror the same information into the store via these tools. This is additive — never skip the markdown calls, and never let the store calls change the prose you write.
