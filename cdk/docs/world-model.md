...
Status: **M9 (cleanup) — final milestone.** M1–M8 are merged; the store is the authoritative
substrate. This doc is the execution checklist for replacing the append-only markdown world-state
logs with a queryable, epistemic, event-sourced store. See **Final architecture (post-M9)** below.
...
| # | Milestone | Mandate |
|---|---|---|
| **M1** | **DONE.** Store substrate: `schema.ts` + `store.ts` + `project.ts` + determinism tests. **Zero pipeline change.** | foundation |
| **M2** | **DONE.** Exporters (`renderCanonContinuity` = load-bearing for press) + legacy importer + press-parity test. *(The legacy importer was later dropped in M9 — never wired.)* | foundation |
| **M3** | **DONE.** Dual-write **shadow**: additive structured tools + `open_chapter`/`close_chapter` frame in **warn-mode**; legacy logs authoritative; divergence logged. + chaos/resume kill-test. *(The shadow framing was retired in M6 when the store became authoritative; its stale comments were dropped in M9.)* | A |
...
| **M9** | **Cleanup — DONE.** (1) Dropped the M3 dual-write **shadow** framing — now-false comments claiming the legacy markdown is authoritative (the store has been authoritative since M6; no active store-vs-markdown diffing ever remained). (2) **Documented** the final state (this status header + consistent DONE marks + the architecture note below). (3) **Dropped the legacy backfill importer** (`import.ts` + its test) rather than wiring it into `cdk run` — a never-wired, never-run, partial migration no active book needs (git history retains it). The load-bearing pieces the grep surfaced were deliberately left intact: the M7 record-drift audit (`findRecordDivergences`), the advisory world-store-coverage check, and the retained LLM re-read. | — |

### Final architecture (post-M9)

The store (`logs/world/events.jsonl`, append-only, projected by `world/project.ts`) is the
**authoritative** continuity substrate; the markdown ledgers (`logs/continuity.md`) are
**regenerated views** of it (`world/regenerate.ts`), guarded so a sparse store never clobbers a
hand-written ledger. The deterministic audit (`world/audit.ts` — fact contradictions, relation
conflicts, the M7 record-drift check, M8 time-indexed semantics) is a precision-perfect **augment**
that runs ALONGSIDE the LLM re-read, which is **retained permanently** (it catches real errors the
