...
3. **Two-gate tension-gradient check (reader-prompt + deterministic kernel).** For each (esp. middle) chapter assert BOTH: **(a)** it changes a NAMED fact or recontextualizes a prior one [accretion]; AND **(b)** the fact it changes was NOT already established in a prior chapter [no re-measurement]. A chapter failing (b) is "orbiting / re-measuring" — emit ONE `structural-failure`/low finding, `repair_agent: null`. A one-gate check waves Test B's ch10 through (it passes (a), fails (b)).
   - **Deterministic kernel (the audit's sweet spot):** ⚠️ **SUPERSEDED (2026-07) — this premise was built, validated against the real stores, and FALSIFIED. See "findOrbitingChapters: investigated and killed" after Validation below.** ~~re-measurement is mechanically detectable in the world store — a chapter whose asserted facts are RE-ASSERTIONS of existing `(entity,attribute)` nodes/values rather than NEW nodes/values, and that registers no new record, is re-measuring. A pure `findOrbitingChapters(tables)` in `audit.ts` flags "this chapter added zero new fact nodes." The judgment half (is a *recontextualization* real vs a re-statement) stays with the reader — the deterministic check only catches the clean no-new-fact case, preserving FP-0.~~ (The reader-prompt two-gate check in 3 above IS shipped and stands; only the *deterministic* kernel is dead.)
...
**Validation.** ALREADY DONE (this is the validated proposal): the 2-book controlled re-run above. Remaining: (i) re-run Test B at matched chapter count (18v18 or 30v30) to strip the length confound — Test A already cleared it. **Operational note from the runs:** a quantity-tracking brief raises the drafter's fact-bookkeeping load enough to blow the default 50-turn cap (cold-signal-mech ch1 needed 76 turns) — ship this alongside a drafter `maxTurns` bump to ~100.

### `findOrbitingChapters`: investigated and killed (2026-07) → replaced by a report-only compounding-rate metric

The pattern to remember: **a deterministic craft signal is a hypothesis about the store, not a fact — validate it against real stores before building the finding.** This one was validated and failed.

**What was proposed** (point 3's kernel above): a chapter that RE-ASSERTS existing `(entity,attribute)` fact-nodes and ADDS none is re-measuring → flag it. Predicted to fire on `cold-signal-mech`'s inventory chapter (ch04) and `coldwater-mech`'s ch10, and stay quiet on the accreting chapters.

**What the real stores showed** (ran the analysis over `the-long-tenant` [reader-DNF sag], `coldwater-reach-mech` + `the-cold-signal-mech` [validated-good middles], `coldwater-m7`/`m6`):
1. **The re-assertion signal does not exist.** The drafter time-stamps quantities INTO the attribute name (`generator.power_reserve.morning_day3`, `matches_remaining_end_ch06`), so a chapter re-walking old ground mints FRESH per-chapter slots instead of re-asserting old ones. Exact-slot re-assertion is ~0 in every store; a `novel-fact == 0` chapter never occurs. The kernel would never fire.
2. **Novel-fact count doesn't discriminate.** The prediction was backwards: `cold-signal-mech` ch04 "the-inventory" registers 24 new facts (among the highest in its book), and the whole book is the STRONGEST compounder. The sag is a reading experience the fact-graph structurally can't see (the project's own "capture is biased toward easy facts; audit-clean ≠ good").

**The salvage — the ONLY signal that discriminated, and it's INVERTED.** Re-touch of an attribute STEM (the same quantity, carried forward: `power_reserve` at day3 → day7) is compounding PRESENCE, not orbiting absence. Aggregated over the middle third it separates ~5–10×: reader-DNF `the-long-tenant` **0.38**, mild braids `coldwater-m7/m6` **0.20 / 0.30**, vs validated-good `coldwater-mech` **1.83** and `cold-signal-mech` **3.20**. (Corroboration: `coldwater-mech` ch10 "no-new-information" scores 0 while its compounding neighbours score 3–4.)

**What shipped instead:** `computeCompoundingStats(tables)` + `attributeStem()` in `src/world/audit.ts` — a **report-only** middle-third compounding rate, printed by `continuity-fact-audit` next to clean-slot rate (LOW note under 0.5). It is **NEVER written to `findings.json` and never handed to a repair agent**: the signal is clean only in aggregate (per-chapter it flags legitimately-quiet chapters — the "a true positive of the machine is a false positive of craft" trap). It is a directional dashboard number for the brief-author, not a gate. Tests: `attributeStem` + `computeCompoundingStats` in `tests/audit.test.ts`.

**Roadmap consequence:** the `findOrbitingChapters` deterministic detector is OFF the roadmap — do not re-propose it. The reader-prompt two-gate check (point 3) and the brief's `## Compounding mechanism` field remain the enforcement path; the report-only rate is the observability layer.
...
- #8 — ~~`findOrbitingChapters`~~ **KILLED (2026-07, see the negative-result note in §8): the re-assertion signal doesn't exist in the real stores.** Replaced by `computeCompoundingStats()` + `attributeStem()` in `src/world/audit.ts` — a **SHIPPED report-only** middle-third compounding rate (inverted signal; ~5–10× separation on the corpus), printed by `continuity-fact-audit`, never a finding.
...
0. **#8 compounding-mechanism brief field + two-gate tension check** (M, high) — ✅ the only EXPERIMENTALLY VALIDATED item (two controlled re-runs on the hard quiet-literary case: 6→7.5–8 and 7→8). Ship the `## Compounding mechanism` brief field + the two-gate "orbiting/re-measurement" reader rule now. (The `findOrbitingChapters` deterministic kernel was investigated 2026-07 and KILLED — the re-assertion signal doesn't exist in real stores; a report-only `computeCompoundingStats` inverted-signal metric shipped in its place. See §8.) Bump drafter `maxTurns` to ~100 alongside it.
...
3. **#1 length FLAG** (S, high) — ✅ **SHIPPED (2026-07): flex-length reframe.** Length is now a story-driven CEILING, not a floor to pad up to, across the brief template (`## Length and shape`), `plotter.md` (step 2 + quality bar), and `drafter.md` (step 6 — target is an upper guide, don't pad); plus a whole-book "padded to length" catch in `editor-pacing.md` macro mode. All carry the **structural-contract floor** (never drop a briefed thread/beat/payoff to come in short — the epic-under-build guard). One-directional: it flags padding, never brevity.
